...
public class SignInWaitingActivity extends Activity {
	...
	protected void onResume() {
		...
		mIntentReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (msg.equalsIgnoreCase("Dear Member, your access token to Justbooksclc app is: "+pas_rand)) {
	...
	public void ManualTypeOtp(){
		count_down.cancel();
		if (timeOut){
			Toast.makeText(getApplicationContext(),"Not able to retrieve your otp sms", Toast.LENGTH_SHORT).show();
		}
		...
		count_down_type = new CountDownTimer(300*1000, 1000) {
			...
			public void onFinish() {
				...
				Toast.makeText(getApplicationContext(),
						"Authentication Timed Out. Please Try Again Later.", Toast.LENGTH_SHORT).show();}
				SignInWaitingActivity.this.finish();
			}
		}.start();
			...
			public void onClick(View v){
        		...
        		if (!otp_code.isEmpty() && otp_code != null){
        			if (otp_code.equals(pas_rand)){
        				...
        				if (timeOut){
    						Toast.makeText(getApplicationContext(),
    								"Authentication Failed Please Try Again Later.", Toast.LENGTH_SHORT).show();}
        				SignInWaitingActivity.this.finish();
        			}
        		}
